...
from os import chdir #for changing directories
...
def html(request,prn):
    #is the text file current and updated? 

    cmd = "yes 'Q' | htlatex 10030142031.tex"
    if get_done(cmd,"%s/%s/" % (RESUME_STORE,prn)):
        #show the HTML file
        t = loader.get_template('/STORE/10030142031/');
        c = Context();
        return t.render(c);
